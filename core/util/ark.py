    ...
    def blocks(self, i='no', val=1, h=None):
        ...
        if i == 'yes':
            try:
                ...
                "burned_fee", "donations" FROM blocks WHERE "generator_public_key" = '{self.PublicKey}' ORDER BY "height" DESC""")
            ...
            try:
                self.cursor.execute(f"""SELECT "id","timestamp","reward","total_fee", "height",
                "burned_fee", "donations" FROM blocks WHERE "generator_public_key" = '{self.PublicKey}' and "height" > {h} ORDER BY "height" DESC LIMIT 250""")
                return self.cursor.fetchall()
            except Exception as e:
                print(e)
